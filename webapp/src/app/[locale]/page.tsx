    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="fixed top-0 w-full bg-white/80 backdrop-blur-md z-50 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/register" className="text-sm bg-primary-600 text-white px-4 py-2 rounded-lg hover:bg-primary-700">{t('nav.getStarted')}</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-32 pb-20 bg-gradient-to-br from-primary-600 via-primary-700 to-purple-700 text-white">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-lg md:text-xl text-primary-100 mb-10 max-w-2xl mx-auto">{t('hero.subtitle')}</p>
          ...
          <div className="max-w-md mx-auto bg-white rounded-2xl shadow-2xl p-6 text-left">
            <div className="flex items-center gap-2 mb-4 pb-3 border-b">
              <span className="w-8 h-8 bg-primary-100 rounded-full flex items-center justify-center text-sm">DL</span>
              <span className="font-semibold text-gray-800 text-sm">{t('chatMock.botName')}</span>
            </div>
            <div className="space-y-3 text-sm">
              <div className="bg-gray-100 text-gray-800 rounded-lg rounded-tl-none p-3 max-w-[80%]">{t('chatMock.greeting')}</div>
              <div className="bg-primary-600 text-white rounded-lg rounded-tr-none p-3 max-w-[80%] ms-auto">{t('chatMock.userMsg')}</div>
              <div className="bg-gray-100 text-gray-800 rounded-lg rounded-tl-none p-3 max-w-[80%]">
                {t('chatMock.correction')} <strong>&quot;{t('chatMock.wrong')}&quot;</strong> → <strong>&quot;{t('chatMock.right')}&quot;</strong><br />
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {steps.map((s) => (
              <div key={s.key} className="text-center">
                <div className="w-14 h-14 bg-primary-600 text-white rounded-full flex items-center justify-center text-xl font-bold mx-auto mb-4">{s.num}</div>
                <h3 className="font-semibold text-gray-800 text-lg mb-2">{t(`howItWorks.${s.key}.title`)}</h3>
                <p className="text-gray-600 text-sm">{t(`howItWorks.${s.key}.desc`)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-8 border border-gray-200">
              ...
              <div className="text-4xl font-bold text-gray-800 mb-6">{t('pricing.free.price')} &euro;<span className="text-base font-normal text-gray-500">{t('pricing.perMonth')}</span></div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 border-2 border-primary-600 relative">
              ...
              <div className="text-4xl font-bold text-gray-800 mb-6">{t('pricing.premium.price')} &euro;<span className="text-base font-normal text-gray-500">{t('pricing.perMonth')}</span></div>
              <ul className="space-y-3 text-sm text-gray-600 mb-8">
                {(t.raw('pricing.premium.features') as string[]).map((feature, i) => (
                  <li key={i}>✓ {feature}</li>
                ))}
              </ul>
